...
-- ── Team Battles ─────────────────────────────────────────────────
-- Challenge multiple friends at once, optionally split into 2 teams.
-- Each client independently computes the current question from elapsed
-- time since start_at (no live broadcast coordination needed). A team
-- scores a question's point only if EVERY member answered it correctly;
-- among teams that cleared that bar, fastest combined answer time wins
-- the point. First to points_to_win wins. Unranked.
...
-- Creates a team battle + invites in one shot. The host needs to create
-- invite rows for OTHER users (the selected friends), which the normal
-- auth.uid() = user_id row policy would block — so this runs as
-- SECURITY DEFINER (inviting friends to a quiz isn't a privileged action).
...
-- clock, so time-based sync (team battle countdown/schedule) isn't
-- thrown off by one device's system clock running a few seconds fast
-- or slow relative to another's.
